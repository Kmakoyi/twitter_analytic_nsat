...
while True:
	...
	Mot_cle =input(" Entrer votre mot_cle : ")
	if not Mot_cle.isdigit() and Mot_cle != string and type(Mot_cle) is str:
		...
		data = pd.read_csv(".\\"+Mot_cle+"_search_Htag_nsat.csv", sep=',',header=None, index_col =0)
		#plt.hist(data, 50)
		data.plot(kind='bar')
		...
		bottom = 0.29
		top = 0.90
		plt.savefig("Histogram" +Mot_cle+".pdf")
